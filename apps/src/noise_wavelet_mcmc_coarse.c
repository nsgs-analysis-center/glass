...
/**
 @file noise_wavelet_mcmc_coarse.c
 \brief Welch/Bartlett-like coarse-grained variant of `noise_wavelet_mcmc`.

 Same data + model machinery, but the WDM time axis is coarse-grained by an
 integer factor Q (--coarse-Q). The data is reduced once (after injection) to
 six channel-pair sufficient statistics P^{(ij)}_{mq}; the model covariance is
 generated directly on the coarse grid every MCMC step, and the likelihood is
 evaluated in the coarse-grained Gaussian sufficient-statistics form.
 */
...
static void print_usage()
{
    ...
    fprintf(stdout,"noise_wavelet_mcmc_coarse --sim-noise --conf-noise --sgwb-template 0 --duration 7864320 --fmin 1e-4 --fmax 8e-3 --coarse-Q 4\n");
    fprintf(stdout,"\n");
    exit(0);
}
...
int main(int argc, char *argv[])
{
    ...
    // Stationary covariance is time-invariant, so any Ncoarse>1 just replicates
    // identical entries through the cov build, the 3x3 inverter, and the LL.
    // Force Ncoarse=1 (Q=NT) for stationary runs.
    ...
    /* read data / inject noise (full-grid wavelet coefficients land in data->tdi) */
    if(flags->strainData)
    ...
    /* Store DFT copy of simulated data for plotting parity with the standard app */
    data->qmin = data->lmin;
    if(!flags->strainData) {
    ...
    write_wdm_data(data, data->tdi->X, "./dbg_X.dat" );

    /* ---- one-shot coarse-graining of the data ---- */
    int Pdim = data->Nlayer * Ncoarse;
    ...
    coarse_grain_wavelet_data(data, Q, Pxx, Pyy, Pzz, Pxy, Pxz, Pyz);
    write_coarse_wdm_data(data, Pxx, "./dbg_coarse_grained_Pxx.dat",Q);

    /* ---- per-chain coarse models ---- */
    printf("   ...initialize instrument noise model\n");
    struct Noise **coarse_scaleogram = malloc(chain->NC*sizeof(struct Noise *));
    ...
    {
        coarse_scaleogram[ic] = malloc(sizeof(struct Noise));
        alloc_noise(coarse_scaleogram[ic], Pdim, data->Nlayer, data->Nchannel);
        coarse_scaleogram[ic]->kmin = data->wdm->kmin;
        // populate frequency axis (constant per layer; replicated across q)
        for (size_t q=0; q<(size_t)Ncoarse; q++)
            for (size_t j=0; j<(size_t)data->Nlayer; j++)
                coarse_scaleogram[ic]->f[q + j*Ncoarse] = (data->lmin + j)*data->wdm->df;
    ...
    /* initial coarse covariance + likelihood */
    for(int ic=0; ic<chain->NC; ic++)
    {
        if (flags->stationary)
            generate_full_stationary_covariance_matrix_coarse(data->wdm, Q, inst_model[ic], conf_model[ic], sgwb_model[ic], coarse_scaleogram[ic]);
        else
            generate_full_dynamic_covariance_matrix_coarse(data->wdm, Q, inst_model[ic], conf_model[ic], sgwb_model[ic], coarse_scaleogram[ic]);

        invert_noise_covariance_matrix(coarse_scaleogram[ic]);
        double logL = my_noise_log_likelihood_wavelet_coarse(data, coarse_scaleogram[ic], Pxx, Pyy, Pzz, Pxy, Pxz, Pyz, Q);
        inst_model[ic]->logL = logL;
        if (sgwb_model[ic]) sgwb_model[ic]->logL = logL;
    ...
    #pragma omp parallel num_threads(flags->threads)
    {
        ...
        for(; step<flags->NMCMC;)
        {
            ...
            {
                struct Noise *psd_ptr = coarse_scaleogram[chain->index[ic]];
                ...
                {
                    noise_instrument_model_mcmc_wavelet_coarse(orbit, data, inst_model_ptr, inst_trial_ptr, conf_model_ptr, sgwb_model_ptr, psd_ptr, Pxx, Pyy, Pzz, Pxy, Pxz, Pyz, Q, chain, flags, ic);
                    if(flags->confNoise)
                        noise_foreground_model_mcmc_wavelet_coarse(data, inst_model_ptr, conf_model_ptr, conf_trial_ptr, sgwb_model_ptr, psd_ptr, Pxx, Pyy, Pzz, Pxy, Pxz, Pyz, Q, chain, flags, ic);
                    if(flags->sgwbTemplate>=0)
                        noise_sgwb_model_mcmc_wavelet_dumb_coarse(data, inst_model_ptr, conf_model_ptr, sgwb_model_ptr, sgwb_trial_ptr, psd_ptr, Pxx, Pyy, Pzz, Pxy, Pxz, Pyz, Q, chain, flags, ic);
                }
            }
            ...
            if(threadID==0)
            {
                ...
                {
                    printf("noise_wavelet_mcmc_coarse at step %i\n",step);
    ...
    free(Pxx); free(Pyy); free(Pzz);
    free(Pxy); free(Pxz); free(Pyz);
