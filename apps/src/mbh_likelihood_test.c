...
/**
 @file mbh_likelihood_test.c
 \brief Main function for testing likelihood functions with app `mbh_likelihood_test`
 */
...
static void parse_mbh_test_likelihood_args(int argc, char **argv, struct Flags *flags)
{
...
static void print_usage()
{
    ...
    fprintf(stdout,"mbh_likelihood_test --inj [inj filename] --sample [chain filename]\n");
    fprintf(stdout,"\n");
...
int main(int argc, char *argv[])
{
    ...
    parse_mbh_test_likelihood_args(argc,argv,flags);
    ...
    */
    ...
    if(flags->verbose)
    {
        ...
        print_mbh_source_params(data, model[0]->source[0], stdout);
        fprintf(stdout,"\n");
    }
    ...
    for(int n=0; n<N; n++)
    {
        ...
        scan_mbh_source_params(data,model[0]->source[0], testFile);
        ...
        print_mbh_source_params(data, model[0]->source[0], outfile);
        ...
        if(flags->verbose)
        {
            ...
            print_wavelet_fourier_spectra(data, model[0]->tdi, filename);
            //fclose(wavfile);
            
            sprintf(filename,"test_residual_%i.dat",n);
            print_wavelet_fourier_spectra(data, model[0]->residual, filename);
