...
if [[ ! -d "$OUTPUT_DIR" ]]; then
    OMP_NUM_THREADS=1 build/apps/src/noise_wavelet_mcmc --sim-noise --steps 100 --chains 1 --cheat --fmin 5e-4 --fmax 8e-3 --duration $((60*60*24*30*1)) --threads 1
...
if [[ ! -d "$OUTPUT_DIR" ]]; then
    OMP_NUM_THREADS=1 build/apps/src/noise_mcmc --sim-noise --steps 100 --chains 1 --cheat --fmin 5e-4 --fmax 8e-3 --duration $((60*60*24*30*1)) --threads 1
